import React from 'react';
import { ShoppingBag, Sparkles, Shirt, User, Camera, Sun, Moon, Cloud, Grid, Heart } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';
import { useCart } from '../contexts/CartContext';

interface NavbarProps {
  wishlistCount: number;
  isNightMode: boolean;
  toggleTheme: () => void;
  isAdminAuthenticated: boolean;
}

const Navbar: React.FC<NavbarProps> = ({ wishlistCount, isNightMode, toggleTheme, isAdminAuthenticated }) => {
  const navigate = useNavigate();
  const { cartCount, toggleCart } = useCart();

  return (
    <nav className="sticky top-4 z-50 mx-4 transition-all duration-500">
      <div className={`rounded-2xl px-4 sm:px-6 py-4 flex justify-between items-center transition-all duration-1000 ${isNightMode ? 'glass-dark' : 'glass'}`}>
        {/* Logo */}
        <div
          className="flex items-center gap-2 cursor-pointer group active:scale-95 transition-transform"
          onClick={() => navigate('/')}
        >
          <div className="relative">
            <Sparkles className={`w-8 h-8 animate-spin-slow ${isNightMode ? 'text-yellow-300' : 'text-moon-400'}`} />
            <div className="absolute top-0 right-0 w-3 h-3 bg-hotpink-500 rounded-full animate-bounce" />
          </div>
          <span className="font-display font-bold text-lg hidden sm:block">Coleções</span>
        </div>

        <div className="hidden md:flex items-center gap-4">
          <Link
            to="/montar-look"
            className="hover:text-hotpink-500 transition-colors flex items-center gap-1 font-bold active:scale-95"
          >
            <Shirt className="w-4 h-4" />
            Montar Look
          </Link>
          <Link
            to="/estudio-magico"
            className={`transition-all active:scale-95 flex items-center gap-1 px-3 py-1 rounded-full border ${isNightMode ? 'bg-purple-900/50 border-purple-700 text-purple-300 hover:text-white' : 'bg-purple-100 border-purple-200 text-purple-500 hover:text-purple-600'}`}
          >
            <Camera className="w-4 h-4" />
            <span className="bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent font-bold">Estúdio Mágico</span>
          </Link>
        </div>

        {/* Actions */}
        <div className="flex items-center gap-3 sm:gap-6">

          {/* THEME TOGGLE */}
          <div className="group relative hidden sm:block">
            <button
              onClick={toggleTheme}
              className={`w-16 h-8 rounded-full p-1 transition-all duration-700 flex items-center shadow-inner relative overflow-hidden active:scale-95 ${isNightMode ? 'bg-indigo-950 border border-indigo-800' : 'bg-sky-200 border border-sky-300'}`}
            >
              {/* Background elements for switch */}
              <div className={`absolute inset-0 transition-opacity duration-700 ${isNightMode ? 'opacity-100' : 'opacity-0'}`}>
                <div className="absolute top-2 left-3 w-0.5 h-0.5 bg-white rounded-full"></div>
                <div className="absolute bottom-2 left-6 w-0.5 h-0.5 bg-white rounded-full"></div>
              </div>
              <div className={`absolute inset-0 transition-opacity duration-700 ${isNightMode ? 'opacity-0' : 'opacity-100'}`}>
                <Cloud className="absolute top-1 right-2 w-3 h-3 text-white fill-white opacity-80" />
              </div>

              {/* The Knob */}
              <div
                className={`w-6 h-6 rounded-full shadow-md transform transition-all duration-700 flex items-center justify-center z-10 ${isNightMode ? 'translate-x-8 bg-moon-300' : 'translate-x-0 bg-yellow-400'}`}
              >
                {isNightMode ? (
                  <Moon className="w-3 h-3 text-indigo-900 fill-indigo-900" />
                ) : (
                  <Sun className="w-4 h-4 text-yellow-100 fill-yellow-100" />
                )}
              </div>
            </button>
          </div>

          <div className="flex items-center gap-2 sm:gap-3">
            <Link
              to="/favoritos"
              className={`relative p-3 rounded-full transition-all active:scale-95 ${isNightMode ? 'hover:bg-white/10 text-gray-300' : 'hover:bg-sky-50 text-gray-500 hover:text-red-500'}`}
              title="Meus Favoritos"
            >
              <Heart className={`w-6 h-6 ${wishlistCount > 0 ? 'fill-red-500 text-red-500' : ''}`} />
              {wishlistCount > 0 && (
                <span className="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full animate-bounce">
                  {wishlistCount}
                </span>
              )}
            </Link>

            <Link
              to={isAdminAuthenticated ? "/admin" : "/login"}
              className={`p-3 rounded-full transition-all active:scale-95 ${isNightMode ? 'hover:bg-white/10 text-gray-300' : 'hover:bg-sky-50 text-gray-500'}`}
            >
              <User className="w-6 h-6" />
            </Link>

            <button
              onClick={toggleCart}
              className={`relative p-3 rounded-xl transition-all shadow-lg active:scale-95 hover:shadow-xl ${isNightMode ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-900/50' : 'bg-sky-400 text-white hover:bg-sky-500 shadow-sky-200'}`}
            >
              <ShoppingBag className="w-6 h-6" />
              {cartCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-hotpink-500 text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-white">
                  {cartCount}
                </span>
              )}
            </button>
          </div>
        </div>
      </div>
    </nav>
  );
};

export default Navbar;